...
//creates a 3d plot  with the z axis being the complex component
...
double input_double (void)
{
    ...
    //this is a misnomer since "scanf()" reads in a float type.
    ...
    if (plot!=NULL )// check that the file was actually opened
    {
        ...
        while (creal(x_c)<uppr)
        {
            while (cimag(x_c)<uppr)
            {
                ...
                //calculate fx
                fx= cabs((eqn[0]*cpow(x_c, 2))+(eqn[1]*x_c)+eqn[2]);
